#!/usr/bin/env python3
"""
Joule Groq-Powered RAG Pipeline with PDF Support
...
def ingest_documents():
    """Ingest documents into vector database (supports .txt and .pdf)"""
    ...
    try:
        ...
        print("💡 Install PDF support: pip install pypdf2")
    
    if not all_docs:
        print("⚠️  No documents found. Place .txt or .pdf files in docs/ directory.")
        return False
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python rag_groq_pdf.py ingest          # Ingest documents (.txt and .pdf)")
        print("  python rag_groq_pdf.py query <question> [context_json]  # Query RAG")
        sys.exit(1)
